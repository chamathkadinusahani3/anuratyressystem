    ...
    <div ref={ref} className="relative flex items-center gap-2 md:gap-3 pl-3 md:pl-6 border-l border-neutral-800">
      <div className="text-right hidden lg:block">
        ...
        <div className="flex items-center justify-end gap-1.5 mt-0.5">
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${roleBadgeClass(user.role as UserRole)}`}>
            {user.role}
          </span>
          {session && !['Super Admin', 'Admin'].includes(user.role) && (
            <span className="text-[10px] text-neutral-500 font-mono">{session.branch}</span>
          )}
        </div>
      </div>
      <button
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <div className="text-center">
        ...
        <p className="text-neutral-500 text-sm">
          ...
          <span className="text-neutral-300 font-medium capitalize">{tab}</span>.
        </p>
      </div>
    </div>
  );
